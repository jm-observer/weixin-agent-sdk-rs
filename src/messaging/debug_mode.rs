...
impl MessageTiming {
    ...
    pub fn platform_to_plugin_ms(&self) -> u64 {
        self.received_at_ms
            .saturating_sub(self.event_timestamp_ms as u64)
    }

    /// Milliseconds spent in inbound processing.
    pub fn inbound_processing_ms(&self) -> Option<u64> {
        self.inbound_done_ms
            .map(|t| t.saturating_sub(self.received_at_ms))
    }
    ...
    pub fn format_report(&self) -> String {
        ...
        parts.push(format!(
            "Platform→Plugin: {}ms",
            self.platform_to_plugin_ms()
        ));
        if let Some(inb) = self.inbound_processing_ms() {
            parts.push(format!("Inbound processing: {}ms", inb));
        }
